...
#ifndef _WAVE_PROPAGATION_H
#define _WAVE_PROPAGATION_H
...
#define COST_FACTOR 3		// Used for translating costs in WavePropagation::setCostmap()
...
*/
   ...
   * @brief Navigation function class. Holds buffers for costmap, WavePropagation map. Maps are pixel-based. Origin is upper left, x is right, y is down. 
   */
  class WavePropagation
  {
    public:
       ...
       */
      WavePropagation(int nx, int ny);	// size of map
       ...
       */
      bool calcWavePropagationAstar();	/**< calculates a plan, returns true if found */

      /**
       * @brief Caclulates the full navigation function using Dijkstra
       */
      bool calcWavePropagationDijkstra(bool atStart = false, bool noLimit = false);	/**< calculates the full navigation function */
      ...
      void setupWavePropagation(bool keepit = false); /**< resets all nav fn arrays for propagation */
       ...
       */
      bool propWavePropagationDijkstra(int cycles, bool atStart = false, bool noLimit = false); /**< returns true if start point found or full prop */
      /**
       ...
       * @return true if the start point is reached
       */
      bool propWavePropagationAstar(int cycles); /**< returns true if start point found */
      ...
      /** display callback */
      void display(void fn(WavePropagation *nav), int n = 100); /**< <n> is the number of cycles between updates  */
      int displayInt;		/**< save second argument of display() above */
      void (*displayFn)(WavePropagation *nav); /**< display function itself */
