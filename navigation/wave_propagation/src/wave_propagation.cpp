...
namespace wave_propagation {
  ...
  int
    ...
    {
      static WavePropagation *nav = NULL;

      if (nav == NULL)
        nav = new WavePropagation(nx,ny);
      ...
      nav->costarr = costmap;
      nav->setupWavePropagation(true);

      // calculate the nav fn and path
      nav->priInc = 2*COST_NEUTRAL;
      nav->propWavePropagationAstar(std::max(nx*ny/20,nx+ny));

      // path
      int len = nav->calcPath(nplan);

      if (len > 0)			// found plan
        ROS_DEBUG("[WavePropagation] Path found, %d steps\n", len);
      else
        ROS_DEBUG("[WavePropagation] No path found\n");
  ...
  WavePropagation::WavePropagation(int xs, int ys)
  {  
    // create cell arrays
    obsarr = costarr = NULL;
    potarr = NULL;
  ...
  void
    WavePropagation::setGoal(int *g)
    {
      goal[0] = g[0];
      goal[1] = g[1];
      ROS_DEBUG("[WavePropagation] Setting goal to %d,%d\n", goal[0], goal[1]);
    }

  void
    WavePropagation::setStart(int *g)
    {
      start[0] = g[0];
      start[1] = g[1];
      ROS_DEBUG("[WavePropagation] Setting start to %d,%d\n", start[0], start[1]);
    }
  ...
  void
    WavePropagation::setNavArr(int xs, int ys)
    {
      ROS_DEBUG("[WavePropagation] Array is %d x %d\n", xs, ys);
  ...
  void
    WavePropagation::setCostmap(const COSTTYPE *cmap, bool isROS, bool allow_unknown)
    {
      COSTTYPE *cm = costarr;
  ...
  bool
  WavePropagation::calcWavePropagationDijkstra(bool atStart, bool noLimit)
    {
      setupWavePropagation(true);

      // calculate the nav fn and path
      double start_time = ros::Time::now().toSec();
      propWavePropagationDijkstra(std::max(nx*ny/20,nx+ny),atStart, noLimit);
      double end_time = ros::Time::now().toSec();
      ROS_INFO("wave propagation takes %.3f", end_time - start_time);
      // path
  ...
  bool
    WavePropagation::calcWavePropagationAstar()
    {
      setupWavePropagation(true);

      // calculate the nav fn and path
      propWavePropagationAstar(std::max(nx*ny/20,nx+ny));
  ...
  float *WavePropagation::getPathX() { return pathx; }
  float *WavePropagation::getPathY() { return pathy; }
  int    WavePropagation::getPathLen() { return npath; }
  ...
  void
    WavePropagation::setObs()
    {
#if 0
      // set up a simple obstacle
      ROS_INFO("[WavePropagation] Setting simple obstacle\n");
      int xx = nx/3;
...
#define push_over(n) { if (n>=0 && n<ns && !pending[n] && \
  ...
  void
    WavePropagation::setupWavePropagation(bool keepit)
    {
  ...
  void
    WavePropagation::initCost(int k, float v)
    {
      potarr[k] = v;
      push_cur(k+1);
      push_cur(k-1);
...
#define INVSQRT2 0.707106781
  ...
  inline void
    WavePropagation::updateCell(int n)
    {
      // get neighbors
      float u,d,l,r;
      l = potarr[n-1];
...
#define INVSQRT2 0.707106781
  ...
  inline void
    WavePropagation::updateCellAstar(int n)
    {
      // get neighbors
      float u,d,l,r;
      l = potarr[n-1];
  ...
  bool
  WavePropagation::propWavePropagationDijkstra(int cycles, bool atStart, bool noLimit)
    {
      ...
      //debug
      ROS_INFO("atStart is %s, noLimit is %s", atStart?("true"):("false"),noLimit?("true"):("false"));
      ...
      ROS_DEBUG("[WavePropagation] Used %d cycles, %d cells visited (%d%%), priority buf max %d\n", 
  ...
  bool
    WavePropagation::propWavePropagationAstar(int cycles)	
    {
      int nwv = 0;			// max priority block size
      int nc = 0;			// number of cells put into priority blocks
      int cycle = 0;		// which cycle we're on
  ...
  float WavePropagation::getLastPathCost()
  {
  ...
  int
    WavePropagation::calcPath(int n, int *st)
    {
      // test write
      //savemap("test");
      ...
      for (int i=0; i<n; i++)
      {
            ...
            oscillation_detected)
        {
          ...
          if (potarr[stc] >= POT_HIGH)
          {
            ...
            //savemap("WavePropagation_highpot");
            return 0;
          }
        }
      ...
      //savemap("WavePropagation_pathlong");
  ...
  float				
    WavePropagation::gradCell(int n)
    {
  ...
  void
    WavePropagation::display(void fn(WavePropagation *nav), int n)
    {
      displayFn = fn;
      displayInt = n;
    }
  ...
  void 
    WavePropagation::savemap(const char *fname)
    {
      char fn[4096];

      ROS_DEBUG("[WavePropagation] Saving costmap and start/goal points");
