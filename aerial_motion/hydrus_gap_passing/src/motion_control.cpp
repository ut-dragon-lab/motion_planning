...
MotionControl::MotionControl(ros::NodeHandle nh, ros::NodeHandle nhp, boost::shared_ptr<TransformController>  transform_controller): nh_(nh), nhp_(nhp)
{
  ...
  //control sub thread
  if(planning_mode_ != hydrus_gap_passing::PlanningMode::ONLY_BASE_MODE)
    {
...
void MotionControl::planStoring(const ompl::base::StateStorage* plan_states, int planning_mode, const std::vector<double>& start_state, const std::vector<double>& goal_state,  double best_cost, double calculation_time)
{
  ...
  state.angle_cos = 1;
  state.angle_sin = 0;
  state.control_mode = hydrus_gap_passing::PlanningMode::POSITION_MODE;


  int state_list = (int)plan_states->size();

  if(planning_mode != hydrus_gap_passing::PlanningMode::ORIGINAL_MODE)
    {
      for(int i = 0; i < state_list; i++)
        {
          if(planning_mode == hydrus_gap_passing::PlanningMode::ONLY_JOINTS_MODE || planning_mode == hydrus_gap_passing::PlanningMode::JOINTS_AND_BASE_MODE)
            {
              if(planning_mode == hydrus_gap_passing::PlanningMode::ONLY_JOINTS_MODE)
                {
              //joints value
              ...
              else if(planning_mode == hydrus_gap_passing::PlanningMode::JOINTS_AND_BASE_MODE)
                {
          ...
          else if(planning_mode == hydrus_gap_passing::PlanningMode::ONLY_BASE_MODE || planning_mode == hydrus_gap_passing::PlanningMode::ONLY_BASE_MODE + hydrus_gap_passing::PlanningMode::ORIGINAL_MODE)
            {
